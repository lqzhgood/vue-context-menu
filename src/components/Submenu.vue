<template>
	<transition name="contextmenu-submenu-fade">
		<div
			:class="['menu', customClass, commonClass.menu]"
			...
			<div class="menu_body">
				<div
					:class="[
                        ...
                        (item.disabled ||  item.children) ? commonClass.unClickableMenuItem : commonClass.clickableMenuItem,
                        (item.children && activeSubmenu.index === index) ? 'menu_item_expand' : null,
                        item.divided ? 'menu_item__divided' : null,
                        item.disabled ? 'menu_item__disabled' :  'menu_item__available',

                    ]"
					:key="item.label"
					@click="(!item.disabled && ! item.children) && itemClick(item)"
					@mouseenter="!item.disabled && enterItem($event,item,index)"
					class="menu_item"
					v-for="(item,index) of itemsShow"
				>
					<div class="menu_item_icon" v-if="hasIcon">
						<i :class="item.icon" v-if="item.icon"></i>
					</div>
					<span class="menu_item_label">{{item.label}}</span>
					<div class="menu_item_expand_icon">{{ item.children ? '▶' : '' }}</div>
				</div>
			</div>
		</div>
	</transition>
...
export default {
	...
	computed: {
		hasIcon() {
			return this.itemsShow.some(v => v.icon);
		},
		itemsShow() {
			return this.items.filter(v => !v.hidden);
		},
	},
	methods: {
